...
The camera latches CAM_LKAS.ERR_BIT_1 ("LKAS Fault: Restart the Car") on a budget of LKAS
requests that produce no effect. Below ~4 m/s a blocked EPS applies exactly zero, so a
saturating lateral command walks straight into that budget; above ~4 m/s the same block
still delivers a third to a half of the request, which is why the latch keys off delivery
rather than off LKAS_BLOCK.
