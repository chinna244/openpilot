...
def build_mon_ver(
  ...
  return add_ubx_checksum(
    b"\xb5\x62\x0a\x04" + len(payload).to_bytes(2, "little") + payload
  )
...
@pytest.mark.parametrize(("mode", "interpretation"), [
  (0, "continuous"),
  (4, "continuous"),
  (1, "power_save"),
  (0x7F, "unknown"),
...
def test_parse_valid_mon_ver_with_multiple_extensions():
  info = parse_mon_ver(build_mon_ver(extensions=(
    "PROTVER=18.00",
    "FWVER=SPG 3.01",
    "MOD=NEO-M8L-0",
    "GPS;GLO;GAL;BDS",
...
def test_mon_ver_rejects_malformed_length_and_bad_checksum():
  ...
  malformed = add_ubx_checksum(
    b"\xb5\x62\x0a\x04" + len(malformed_payload).to_bytes(2, "little") + malformed_payload
  )
...
def test_receiver_identity_normalization_is_deterministic():
  ...
  assert normalized_receiver_identity(first) == (
    "sw=ext core 3.01|hw=00080000|ext=gps;protver=18.00"
  )
...
def test_mon_ver_is_polled_each_receiver_cycle_and_failure_does_not_block_assistance(
  ...
  process_start_result = pigeond.initialize_receiver_cycle(
    pigeon, "receiver", diagnostics, "process_start",
    collect_mon_ver_diagnostics=True,
  )
  recovery_result = pigeond.initialize_receiver_cycle(
    pigeon, "receiver", diagnostics, "no_data_watchdog",
  )
...
def test_acquisition_diagnostics_run_only_on_initial_diagnostic_cycle_and_run_last(monkeypatch):
  ...
  monkeypatch.setattr(pigeond, "init", lambda pigeon: calls.append("init"))
  monkeypatch.setattr(
    pigeond, "log_mon_ver_diagnostics",
    lambda pigeon: calls.append("mon_ver") or None,
  )
  monkeypatch.setattr(
    pigeond, "poll_mon_ver",
    lambda pigeon: calls.append("mon_ver") or None,
  )
  monkeypatch.setattr(
    pigeond, "log_navx5_ack_aiding_support",
    lambda info: calls.append("ack_support"),
  )
  monkeypatch.setattr(
    pigeond, "configure_navx5_ack_aiding",
  ...
  monkeypatch.setattr(
    pigeond, "log_assistnow_autonomous_support",
    lambda info: calls.append("aop_support") or False,
  )
  monkeypatch.setattr(
    pigeond, "configure_assistnow_autonomous",
    lambda pigeon, info: calls.append("aop_config"),
  )
  monkeypatch.setattr(
    pigeond, "log_acquisition_configuration_diagnostics",
    lambda pigeon, info: calls.append("acquisition_diagnostics"),
  )

  diagnostics = Diagnostics()
  pigeond.initialize_receiver_cycle(
    object(), "receiver", diagnostics, "process_start",
    collect_mon_ver_diagnostics=True,
  )
  pigeond.initialize_receiver_cycle(
    object(), "receiver", diagnostics, "no_data_watchdog",
...
def test_acquisition_diagnostic_failure_does_not_block_initialized_result(monkeypatch):
  ...
  assert calls == ["time_assistance", "restore", "acquisition_diagnostics"]
...
def test_restore_summary_success_partial_timeout_and_no_attempt():
  success = pigeond.NavigationAssistanceRestoreResult(
    pigeond.NavigationAssistanceRestoreStatus.COMPLETE, 3, 3,
  )
  ...
  success_text = pigeond.format_navigation_assistance_restore_summary(
    success, attempted=True, time_assistance_source="synchronized",
  )
  ...
  partial_text = pigeond.format_navigation_assistance_restore_summary(
    partial, attempted=True, time_assistance_source="rtc_estimate",
  )
  ...
  timeout_text = pigeond.format_navigation_assistance_restore_summary(
    timeout, attempted=True, time_assistance_source="synchronized",
  )
  ...
  no_attempt_text = pigeond.format_navigation_assistance_restore_summary(
    None, attempted=False, time_assistance_source=None,
  )
