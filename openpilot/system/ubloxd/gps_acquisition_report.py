...
def _milestone_fields(metrics: Any) -> dict[str, float | None]:
  return {
    ...
    "first_fix_legacy_flags": relative_seconds(metrics, getattr(metrics, "first_fix_legacy_flags", None)),
...
def ttff_report_lines(metrics: Any) -> list[str]:
  ...
  lines = [
    ...
    f"has_qcom_gnss={getattr(metrics, 'has_qcom_gnss', False)}",
    f"ttff_fix_policy={getattr(metrics, 'ttff_fix_policy', 'strict_has_fix')}",
