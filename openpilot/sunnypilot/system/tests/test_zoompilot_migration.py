...
@pytest.mark.parametrize("origin", [ZOOMPILOT_ORIGIN, "git@github.com:zoompilot/zoompilot.git"])
@pytest.mark.parametrize("branch", LEGACY_BRANCHES)
def test_targets_main_without_rewriting_an_already_migrated_origin(monkeypatch, origin, branch):
...
@pytest.mark.parametrize("origin", [
  ...
  "https://github.com/someoneelse/openpilot.git",   # another fork carrying the same branch name
  "git@github.com:someoneelse/sunnypilot.git",
  "https://github.com/zephleggett/sunnypilot.git",  # same account, never an install source
])
@pytest.mark.parametrize("branch", LEGACY_BRANCHES)
def test_foreign_origin_on_a_legacy_branch_name_is_left_alone(monkeypatch, origin, branch):
  """A legacy branch NAME on someone else's fork is not our install: neither the
  origin nor the target branch may be touched."""
  params = Params()
  params.remove("UpdaterTargetBranch")
  calls = _setup(monkeypatch, origin, branch)
  ...
  try:
    _migrate_zoompilot_channel(params)
    assert calls == [], "foreign origin was rewritten"
    assert params.get("UpdaterTargetBranch") is None, "foreign install was retargeted"
  finally:
    params.remove("UpdaterTargetBranch")
