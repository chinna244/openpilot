...
def _migrate_zoompilot_channel(_params):
  ...
  # The branch name alone is not ours to claim: an unrelated fork can carry a
  # "mazda-dev" or "zoompilot" branch too, and retargeting it would drag that
  # device onto our release channel. Only an origin we shipped from (or one
  # already repointed at us) qualifies for either half of the migration.
  normalized_origin = _normalize_origin(origin)
  if normalized_origin not in LEGACY_ORIGINS and normalized_origin != _normalize_origin(ZOOMPILOT_ORIGIN):
    return

  if normalized_origin in LEGACY_ORIGINS:
