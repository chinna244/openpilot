...
def _make_mads(mocker, CP, CP_SP, *, uem=True):
  ...
  sd.sm = FakeSubMaster({"pandaStates": [ps]})
...
class TestMazdaPandaAuthPause:
  def test_auth_loss_counts_after_grace_but_keeps_mads(self, mocker):
    from openpilot.sunnypilot.mads.mads import MAZDA_LATERAL_AUTH_GRACE, LATERAL_MISMATCH_DISABLE_FRAMES
    ...
    for _ in range(LATERAL_MISMATCH_DISABLE_FRAMES):
      h.mads.data_sample()
    assert h.mads.lateral_mismatch_counter == MAZDA_LATERAL_AUTH_GRACE + LATERAL_MISMATCH_DISABLE_FRAMES - 1
    assert h.mads.enabled
    h.mads.data_sample()
    assert h.mads.lateral_mismatch_counter == MAZDA_LATERAL_AUTH_GRACE + LATERAL_MISMATCH_DISABLE_FRAMES
  ...
  def test_missing_panda_counts_as_mismatch(self, mocker):
    ...
    h.mads.active = True
    h.sd.sm["pandaStates"] = []
    for _ in range(MAZDA_LATERAL_AUTH_GRACE + LATERAL_MISMATCH_DISABLE_FRAMES):
      h.mads.data_sample()
    h.mads.update_events(structs.CarState())
    assert h.sd.events_sp.has(EventNameSP.controlsMismatchLateral)
    assert h.mads.enabled
  ...
  def test_set_car_specific_params_disables_mazda_uem(self, mocker):
    ...
    CP.brand = "mazda"
    sc = mocker.MagicMock()
    sc.safetyParam = MazdaSafetyFlags.TJA_MADS
    CP.safetyConfigs = [sc]
  ...
  def test_set_car_specific_params_leaves_non_tja_mazda(self, mocker):
    params = mocker.MagicMock()
    for safety_param in (0, MazdaSafetyFlags.STEER_TO_ZERO_EPS, MazdaSafetyFlags.LONG):
      params.reset_mock()
      CP = mocker.MagicMock()
      CP.brand = "mazda"
      sc = mocker.MagicMock()
      sc.safetyParam = safety_param
      CP.safetyConfigs = [sc]
      set_car_specific_params(CP, mocker.MagicMock(), params)
      assert not any(call.args[:2] == ("MadsUnifiedEngagementMode", False)
                     for call in params.put_bool.call_args_list)
      # brand=="mazda" alone must never remove MadsMainCruiseAllowed for TJA reasons;
      # only the shared MADS_NO_ACC_MAIN_BUTTON path (not mazda) may remove it.
      assert not any(call.args == ("MadsMainCruiseAllowed",)
                     for call in params.remove.call_args_list)

  def test_mads_init_ignores_steer_to_zero_without_tja_mads(self, mocker):
    """STEER_TO_ZERO / brand mazda must not force TJA-only MADS settings."""
    CP = mocker.MagicMock()
    CP.brand = "mazda"
    CP.passive = False
    sc = mocker.MagicMock()
    sc.safetyParam = MazdaSafetyFlags.STEER_TO_ZERO_EPS
    CP.safetyConfigs = [sc]
    mads, _sd = _make_mads(mocker, CP, mocker.MagicMock(), uem=True)
    assert not mads.tja_mads
    assert not mads.allow_always
    assert not mads.no_main_cruise
    assert mads.main_enabled_toggle is True
    assert mads.unified_engagement_mode is True
  ...
  def test_panda_lateral_allowed_helper(self):
    ...
    def sm(pandas, *, mads_active=True, selfdrive_active=False, checks_ok=True):
    ...
    # Healthy + fresh with lateral auth.
    assert fn(sm([PS("mazda", cal=True)], checks_ok=True)) is True
    # Stale/dead service must deny even with a cached controlsAllowedLateral=True.
    assert fn(sm([PS("mazda", cal=True)], checks_ok=False)) is False
    # Invalid / failed SubMaster checks deny immediately.
    assert fn(sm([PS("mazda", cal=True, ca=True)], checks_ok=False)) is False
